...
- **复习日志已接入云同步**（`korean_wordlist_review_log`，90 天本地修剪窗口）：走 blob key `wordlist_review_log`，合并型是 **set**（按 id+time 并集、append-only、无墓碑）——不能用 arr 型，arr 的按 id 后写胜出会把「同一词条多日复习」折叠成一条。真正的同步闸门有两道：前端 `js/sync.js` 的 `SYNC_BLOB_MAP` + 服务端 `functions/api/_shared.js` 的 `BLOB_KEYS` 白名单，**两端都要加**漏一边就是静默丢弃（服务端 `user_blobs` 是通用键值表，无需建表/迁移；早期文档「需给 D1 加表」的机制描述已过时）。本地回归：`node tests/e2e/review-log-sync.js`（独立 pages dev + 独立 D1 persist）。
